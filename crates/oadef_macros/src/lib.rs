...
#[proc_macro_derive(new, attributes(oadef))]
    ...
    fn derive_new(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
        ...
        let AttrVal(prog_id) = attrs
            ...
            .flat_map(|attr| match &attr.meta {
                Meta::List(MetaList { path, .. }) if path.is_ident("oadef") => {
            ...
            .exactly_one()
            .map_err(|_| {
                syn::Error::new(proc_macro2::Span::call_site(), "missing `#[oadef(…)]`")
            })??;
        ...
        Ok(quote! {
            impl #impl_generics #ident #ty_generics #where_clause {
                #vis fn new() -> ::oadef::Result<Self> {
                    let disp = ::oadef::idispatch_from_prog_id(::windows::core::w!(#prog_id))?;
...
pub fn derive_try_from_out(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    fn derive_try_from_out(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
        ...
        Ok(quote! {
            impl #impl_generics ::std::convert::TryFrom<::oadef::Out> for #ident #ty_generics #where_clause {
                type Error = ::oadef::Error;

                fn try_from(out: ::oadef::Out) -> ::oadef::Result<Self> {
